...
def _normalize_aliases(data: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize top-level alias keys to their canonical forms.

    Supports user-friendly aliases so configs generated by autoconfigure
    or written from documentation examples work without warnings:
      - 'languages' -> 'project.languages'
      - 'domains' -> 'pipeline' (merged with existing pipeline config)
    ...
    Returns:
        Normalized configuration dictionary with aliases resolved.
    """
    result = data.copy()

    # 'languages' -> project.languages
    if "languages" in result:
        languages = result.pop("languages")
        project = result.get("project", {})
        if not isinstance(project, dict):
            project = {}
        # Only set if not already specified under project
        if "languages" not in project:
            project["languages"] = languages
        result["project"] = project

    # 'domains' -> pipeline (merge domain configs into pipeline)
    if "domains" in result:
        domains = result.pop("domains")
        if isinstance(domains, dict):
            pipeline = result.get("pipeline", {})
            if not isinstance(pipeline, dict):
                pipeline = {}
            # Merge each domain key from 'domains' into 'pipeline',
            # but don't overwrite keys already present in pipeline
            pipeline = merge_configs(domains, pipeline)
            result["pipeline"] = pipeline

    # 'exclude_patterns' -> 'exclude'
    if "exclude_patterns" in result:
        exclude_patterns = result.pop("exclude_patterns")
        # Only set if neither 'exclude' nor 'ignore' is already specified
        if "exclude" not in result and "ignore" not in result:
            result["exclude"] = exclude_patterns
...
def dict_to_config(data: Dict[str, Any]) -> LucidSharkConfig:
    ...
    """
    # Normalize alias keys before processing
